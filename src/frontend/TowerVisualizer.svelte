<script>
    ...
    // Add test towers when no layers are provided
    $: if (!layers || layers.length === 0) {
        layers = createTestTower();
    }

    function createTestTower() {
        // Create a valid tower with multiple floors and directions
        const testLayers = [];

        // Ground floor (z=0)
        const layer0 = createMockLayer(5, 5);
        // Horizontal word БАШНЯ in X direction
        layer0.grid[2][1] = { char: "Б", dir: 2 };
        layer0.grid[2][2] = { char: "А", dir: 2 };
        layer0.grid[2][3] = { char: "Ш", dir: 2 };
        layer0.grid[2][4] = { char: "Н", dir: 2 };
        layer0.grid[2][5] = { char: "Я", dir: 2 };

        // Horizontal word СЛОВО in Y direction
        layer0.grid[0][2] = { char: "С", dir: 3 };
        layer0.grid[1][2] = { char: "Л", dir: 3 };
        layer0.grid[2][2] = { char: "О", dir: 3 }; // Intersection with БАШНЯ at А->О
        layer0.grid[3][2] = { char: "В", dir: 3 };
        layer0.grid[4][2] = { char: "О", dir: 3 };

        testLayers.push(layer0);

        // First floor below (z=-1)
        const layer1 = createMockLayer(5, 5);
        // Horizontal word ИГРА in X direction
        layer1.grid[1][3] = { char: "И", dir: 2 };
        layer1.grid[1][4] = { char: "Г", dir: 2 };
        layer1.grid[1][5] = { char: "Р", dir: 2 };
        layer1.grid[1][6] = { char: "А", dir: 2 };
        testLayers.push(layer1);

        // Second floor below (z=-2)
        const layer2 = createMockLayer(5, 5);
        // Horizontal word ЭТАЖ in X direction
        layer2.grid[3][1] = { char: "Э", dir: 2 };
        layer2.grid[3][2] = { char: "Т", dir: 2 };
        layer2.grid[3][3] = { char: "А", dir: 2 };
        layer2.grid[3][4] = { char: "Ж", dir: 2 };
        testLayers.push(layer2);

        // Now add vertical words
        // Vertical word БАШНЯ from z=0 down to z=-4
        const layer3 = createMockLayer(5, 5);
        layer3.grid[2][1] = { char: "Б", dir: 1 }; // Already shown on layer0
        testLayers[1].grid[2][1] = { char: "А", dir: 1 }; // layer1 z=-1
        testLayers[2].grid[2][1] = { char: "Ш", dir: 1 }; // layer2 z=-2
        layer3.grid[2][1] = { char: "Н", dir: 1 }; // z=-3
        testLayers.push(layer3);

        const layer4 = createMockLayer(5, 5);
        layer4.grid[2][1] = { char: "Я", dir: 1 }; // z=-4
        testLayers.push(layer4);

        // Vertical word СТОЛ from z=0 down to z=-3
        testLayers[1].grid[0][2] = { char: "Т", dir: 1 }; // layer1 z=-1
        testLayers[2].grid[0][2] = { char: "О", dir: 1 }; // layer2 z=-2
        testLayers[3].grid[0][2] = { char: "Л", dir: 1 }; // layer3 z=-3
                ...
                for (let x = 0; x < width; x++) {
                    ...
                    if (
                        ...
                        if (!charTexture) {
                            ...
                            if (charTexture)
                                characterTextures.set(char, charTexture);
                        }
    ...
    onMount(() => {
        ...
        materialOverlap = new THREE.MeshStandardMaterial({
            color: 0xffff00,
            metalness: 0.2,
            roughness: 0.7,
        });
        ...
        materialX = new THREE.MeshStandardMaterial({
            color: 0xff6666,
            metalness: 0.3,
            roughness: 0.6,
        });
        materialY = new THREE.MeshStandardMaterial({
            color: 0x66ff66,
            metalness: 0.3,
            roughness: 0.6,
        });
        materialZ = new THREE.MeshStandardMaterial({
            color: 0x6666ff,
            metalness: 0.3,
            roughness: 0.6,
        });
        ...
        resizeObserver = new ResizeObserver((entries) => {
            if (!entries || entries.length === 0 || !renderer || !camera)
                return;
...
<div class="visualizer-container" bind:this={canvasContainer}></div>
